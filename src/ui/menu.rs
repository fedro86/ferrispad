...
pub fn build_menu(
    ...
    // Edit
    menu.add("Edit/Undo", Shortcut::Ctrl | 'z', MenuFlag::Normal, { let s = s.clone(); move |_| s.send(Message::EditUndo) });
    menu.add("Edit/Redo", Shortcut::Ctrl | Shortcut::Shift | 'z', MenuFlag::Normal, { let s = s.clone(); move |_| s.send(Message::EditRedo) });
    ...
    menu.add("View/Toggle Line Numbers", Shortcut::None, ln_flag, { let s = s.clone(); move |_| s.send(Message::ToggleLineNumbers) });
    let ww_flag = if settings.word_wrap_enabled { MenuFlag::Toggle | MenuFlag::Value } else { MenuFlag::Toggle };
    menu.add("View/Toggle Word Wrap", Shortcut::None, ww_flag, { let s = s.clone(); move |_| s.send(Message::ToggleWordWrap) });
    ...
    menu.add("Format/Font/Courier", Shortcut::None, MenuFlag::Normal, { let s = s.clone(); move |_| s.send(Message::SetFont(Font::Courier)) });
    menu.add("Format/Font/Helvetica Mono", Shortcut::None, MenuFlag::Normal, { let s = s.clone(); move |_| s.send(Message::SetFont(Font::Screen)) });
    menu.add("Format/Font Size/Small (12)", Shortcut::None, MenuFlag::Normal, { let s = s.clone(); move |_| s.send(Message::SetFontSize(12)) });
    menu.add("Format/Font Size/Medium (16)", Shortcut::None, MenuFlag::Normal, { let s = s.clone(); move |_| s.send(Message::SetFontSize(16)) });
    menu.add("Format/Font Size/Large (20)", Shortcut::None, MenuFlag::Normal, { let s = s.clone(); move |_| s.send(Message::SetFontSize(20)) });

    // Help
    menu.add("Help/About FerrisPad", Shortcut::None, MenuFlag::Normal, { let s = s.clone(); move |_| s.send(Message::ShowAbout) });
    menu.add("Help/Check for Updates...", Shortcut::None, MenuFlag::Normal, { let s = s.clone(); move |_| s.send(Message::CheckForUpdates) });
}

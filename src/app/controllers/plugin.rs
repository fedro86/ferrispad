...
impl PluginController {
    ...
    /// Returns true if permissions were approved and a re-highlight should be triggered.
    pub fn request_plugin_permissions(
        ...
        match show_permission_dialog(&request) {
            ApprovalResult::Approved(cmds) => {
                if let Some(plugin) = plugins.plugins_mut().iter_mut().find(|p| p.name == name) {
                    plugin.approved_commands.extend(cmds.clone());
                    plugin.enabled = true;
                }
                {
            ...
            ApprovalResult::Cancelled => {
                eprintln!("[plugins] {} permission dialog cancelled", name);
                false
            }
        }
    }

...
- Minimal dependency surface reduces attack vectors and simplifies security auditing.
...
- **Sandboxed by default:** Plugins operate within strict boundaries—no arbitrary file access, no unrestricted command execution. User consent required for sensitive operations.

## 6. Privacy & Security are Binary
...
- **Memory Safety:** Rust's ownership model eliminates entire classes of vulnerabilities (buffer overflows, use-after-free). We minimize `unsafe` code and treat all external input as untrusted.
